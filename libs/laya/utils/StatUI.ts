...
export class StatUI extends IStatRender {
	...
	show(x: number = 0, y: number = 0): void {
		var dt: any = Stat;
		if (!Browser.onMiniGame && !ILaya.Render.isConchApp && !Browser.onBDMiniGame && !Browser.onKGMiniGame && !Browser.onQGMiniGame && !Browser.onQQMiniGame && !Browser.onAlipayMiniGame && !Browser.onBLMiniGame) this._useCanvas = true;
		this._show = true;
