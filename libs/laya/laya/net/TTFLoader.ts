...
export class TTFLoader {
    ...
    private _checkComplete(): void {
        if (
            ILaya.Browser.measureText(TTFLoader._testString, this._fontTxt)
                .width != this._txtWidth
        ) {
            this._complete();
        }
    }
    ...
    private _loadWithFontFace(): void {
        var fontFace: any = new (window as any).FontFace(
            this.fontName,
            "url('" + this._url + "')",
        );
    ...
    private _loadWithCSS(): void {
        ...
        this._txtWidth = Browser.measureText(
            TTFLoader._testString,
            this._fontTxt,
        ).width;
