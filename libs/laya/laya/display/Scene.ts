...
export class Scene extends Sprite {
    ...
    constructor(createChildren = true) {
        ...
        this._scene = this;
        if (createChildren) this.createChildren();
    }
    ...
    static setUIMap(url): void {
        ...
        if (uimap) {
            for (let key in uimap) {
                ILaya.Loader.loadedMap[URL.formatURL(key + '.scene')] =
                    uimap[key];
            }
        } else {
    ...
    static load(
        ...
        function create(): void {
            ...
            var runtime: string = obj.props.runtime
                ? obj.props.runtime
                : obj.type;
            var clas: any = ILaya.ClassUtils.getClass(runtime);
            if (obj.props.renderType == 'instance') {
                var scene: Scene =
                    clas.instance || (clas.instance = new clas());
            } else {
                scene = new clas();
            }
    ...
    static open(
        ...
        Scene.load(
            url,
            Handler.create(null, this._onSceneLoaded, [
                closeOther,
                complete,
                param,
            ]),
