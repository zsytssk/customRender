...
export class Dragging {
    ...
    private loop(): void {
        ...
        if (this._clickOnly) {
            if (
                Math.abs(offsetX * ILaya.stage._canvasTransform.getScaleX()) >
                    1 ||
                Math.abs(offsetY * ILaya.stage._canvasTransform.getScaleY()) > 1
            ) {
    ...
    private checkArea(): void {
        if (this.elasticDistance <= 0) {
            ...
            this._elasticRateX = Math.max(
                0,
                1 - offsetX / this.elasticDistance,
            );
            ...
            this._elasticRateY = Math.max(
                0,
                1 - offsetY / this.elasticDistance,
            );
        }
    }
