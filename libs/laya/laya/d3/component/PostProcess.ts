...
export class PostProcess {
    ...
    /**@internal */
    static SHADERVALUE_BLOOMTEX: number = Shader3D.propertyNameToID(
        'u_BloomTex',
    );
    /**@internal */
    static SHADERVALUE_AUTOEXPOSURETEX: number = Shader3D.propertyNameToID(
        'u_AutoExposureTex',
    );
    /**@internal */
    static SHADERVALUE_BLOOM_DIRTTEX: number = Shader3D.propertyNameToID(
        'u_Bloom_DirtTex',
    );
    /**@internal */
    static SHADERVALUE_BLOOMTEX_TEXELSIZE: number = Shader3D.propertyNameToID(
        'u_BloomTex_TexelSize',
    );
    /**@internal */
    static SHADERVALUE_BLOOM_DIRTTILEOFFSET: number = Shader3D.propertyNameToID(
        'u_Bloom_DirtTileOffset',
    );
    /**@internal */
    static SHADERVALUE_BLOOM_SETTINGS: number = Shader3D.propertyNameToID(
        'u_Bloom_Settings',
    );
    /**@internal */
    static SHADERVALUE_BLOOM_COLOR: number = Shader3D.propertyNameToID(
        'u_Bloom_Color',
    );
    ...
    static __init__(): void {
        PostProcess.SHADERDEFINE_BLOOM_LOW = Shader3D.getDefineByName(
            'BLOOM_LOW',
        );
        PostProcess.SHADERDEFINE_BLOOM = Shader3D.getDefineByName('BLOOM');
        PostProcess.SHADERDEFINE_FINALPASS = Shader3D.getDefineByName(
            'FINALPASS',
        );
    }
    ...
    _render(): void {
        ...
        ILaya.Render.supportWebGLPlusRendering &&
            ShaderData.setRuntimeValueMode(false);

        var camera: Camera = this._context.camera;
        var viewport: Viewport = camera.viewport;

        var screenTexture: RenderTexture = RenderTexture.createFromPool(
            RenderContext3D.clientWidth,
            RenderContext3D.clientHeight,
            camera._getRenderTextureFormat(),
        ...
        this._context.compositeShaderData.setTexture(
            PostProcess.SHADERVALUE_AUTOEXPOSURETEX,
        ...
        var canvasWidth: number = camera._getCanvasWidth(),
            canvasHeight: number = camera._getCanvasHeight();
        camera._screenOffsetScale.setValue(
            viewport.x / canvasWidth,
            viewport.y / canvasHeight,
            viewport.width / canvasWidth,
            viewport.height / canvasHeight,
        );
        this._context.command.blitScreenTriangle(
            this._context.source,
            dest,
            camera._screenOffsetScale,
            this._compositeShader,
        ...
        var tempRenderTextures: RenderTexture[] = this._context
