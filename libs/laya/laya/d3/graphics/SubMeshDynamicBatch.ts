...
export class SubMeshDynamicBatch extends GeometryElement {
    ...
    constructor() {
        ...
        var maxVerDec: VertexDeclaration = VertexMesh.getVertexDeclaration(
            'POSITION,NORMAL,COLOR,UV,UV1,TANGENT',
        );
        var maxByteCount: number =
            maxVerDec.vertexStride * SubMeshDynamicBatch.maxIndicesCount;
        ...
        this._indexBuffer = new IndexBuffer3D(
            IndexFormat.UInt16,
        ...
        var memorySize: number =
            this._vertexBuffer._byteLength + this._indexBuffer._byteLength;
    ...
    private _getBatchVertices(
        ...
        batchVertices: Float32Array,
        batchOffset: number,
        transform: Transform3D,
        element: SubMeshRenderElement,
        subMesh: SubMesh,
    ): void {
        ...
        element._computeWorldPositionsAndNormals(
    ...
    private _getBatchIndices(
        ...
        if (multiSubMesh) {
            ...
            if (isInvert) {
                for (k = 0, m = subIndices.length; k < m; k += 3) {
                    batchOffset = batchIndexCount + k;
                    batchIndices[batchOffset] =
                        batchVertexCount + subIndices[k];
                    batchIndices[batchOffset + 1] =
                        batchVertexCount + subIndices[k + 2];
                    batchIndices[batchOffset + 2] =
                        batchVertexCount + subIndices[k + 1];
                }
            } else {
                for (k = 0, m = subIndices.length; k < m; k += 3) {
                    batchOffset = batchIndexCount + k;
    ...
    private _flush(vertexCount: number, indexCount: number): void {
        ...
        this._vertexBuffer.setData(
            ...
            0,
            0,
            vertexCount * this._bufferState.vertexDeclaration.vertexStride,
        );
    ...
    _prepareRender(state: RenderContext3D): boolean {
        var element: SubMeshRenderElement = <SubMeshRenderElement>(
            state.renderElement
        );
        var vertexDeclaration: VertexDeclaration =
            element.vertexBatchVertexDeclaration;
        this._bufferState = ILaya3D.MeshRenderDynamicBatchManager.instance._getBufferState(
            vertexDeclaration,
        );

        this._positionOffset =
            vertexDeclaration.getVertexElementByUsage(VertexMesh.MESH_POSITION0)
                ._offset / 4;
        var normalElement: VertexElement = vertexDeclaration.getVertexElementByUsage(
            VertexMesh.MESH_NORMAL0,
        );
        this._normalOffset = normalElement ? normalElement._offset / 4 : -1;
        var colorElement: VertexElement = vertexDeclaration.getVertexElementByUsage(
            VertexMesh.MESH_COLOR0,
        );
        this._colorOffset = colorElement ? colorElement._offset / 4 : -1;
        var uv0Element: VertexElement = vertexDeclaration.getVertexElementByUsage(
            VertexMesh.MESH_TEXTURECOORDINATE0,
        );
        this._uv0Offset = uv0Element ? uv0Element._offset / 4 : -1;
        var uv1Element: VertexElement = vertexDeclaration.getVertexElementByUsage(
            VertexMesh.MESH_TEXTURECOORDINATE1,
        );
        this._uv1Offset = uv1Element ? uv1Element._offset / 4 : -1;
        var tangentElement: VertexElement = vertexDeclaration.getVertexElementByUsage(
    ...
    _render(context: RenderContext3D): void {
        ...
        var vertexDeclaration: VertexDeclaration =
            element.vertexBatchVertexDeclaration;
        var batchElements: SingletonList<SubMeshRenderElement> =
            element.vertexBatchElementList;
        ...
        for (var i: number = 0; i < elementCount; i++) {
            ...
            this._getBatchVertices(
                vertexDeclaration,
                this._vertices,
                batchVertexCount,
                transform,
                /*(element.render as MeshRender)*/ subElement,
                subMesh,
            );
            this._getBatchIndices(
                this._indices,
                batchIndexCount,
                batchVertexCount,
                transform,
                subMesh,
                subElement._dynamicMultiSubMesh,
            );
