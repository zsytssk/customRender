...
} from '../../resource/RenderTextureFormat';
...
export class ParallelSplitShadowMap {
    ...
    /**@internal */
    private _boundingSphere: BoundSphere[] = new Array<BoundSphere>(
        ParallelSplitShadowMap.MAX_PSSM_COUNT + 1,
    );
    /**@internal */
    _boundingBox: BoundBox[] = new Array<BoundBox>(
        ParallelSplitShadowMap.MAX_PSSM_COUNT + 1,
    );
    /**@internal */
    private _frustumPos: Vector3[] = new Array<Vector3>(
        (ParallelSplitShadowMap.MAX_PSSM_COUNT + 1) * 4,
    );
    /**@internal */
    private _uniformDistance: number[] = new Array<number>(
        ParallelSplitShadowMap.MAX_PSSM_COUNT + 1,
    );
    /**@internal */
    private _logDistance: number[] = new Array<number>(
        ParallelSplitShadowMap.MAX_PSSM_COUNT + 1,
    );
    /**@internal */
    private _dimension: Vector2[] = new Array<Vector2>(
        ParallelSplitShadowMap.MAX_PSSM_COUNT + 1,
    );
    /** @internal */
    ...
    /**@internal */
    private _splitFrustumCulling: BoundFrustum = new BoundFrustum(
        Matrix4x4.DEFAULT,
    );
    /** @internal */
    ...
    setPCFType(PCFtype: number): void {
        ...
        switch (this._PCFType) {
            case 0:
                defineData.addDefine(
                    Scene3DShaderDeclaration.SHADERDEFINE_SHADOW_PCF_NO,
                );
                defineData.removeDefine(
                    Scene3DShaderDeclaration.SHADERDEFINE_SHADOW_PCF1,
                );
                defineData.removeDefine(
                    Scene3DShaderDeclaration.SHADERDEFINE_SHADOW_PCF2,
                );
                defineData.removeDefine(
                    Scene3DShaderDeclaration.SHADERDEFINE_SHADOW_PCF3,
                );
                break;
            case 1:
                defineData.addDefine(
    ...
    set shadowMapCount(value: number) {
        ...
        value =
            value <= ParallelSplitShadowMap.MAX_PSSM_COUNT
                ? value
                : ParallelSplitShadowMap.MAX_PSSM_COUNT;
        if (this._shadowMapCount != value) {
            ...
            for (var i: number = 0; i < value; i++)
                this._shaderValueVPs[i] = new Float32Array(
    ...
    _calcAllLightCameraInfo(sceneCamera: BaseCamera): void {
        if (this._shadowMapCount === 1) {
            ...
            for (
                var i: number = 0, n: number = this._shadowMapCount + 1;
                i < n;
                i++
            ) {
    ...
    private _update(sceneCamera: BaseCamera): void {
        ...
        if (
            ...
            //TODO:同一场景多摄像机频繁切换仍会重新计算,将包围矩阵存到摄像机自身可解决
    ...
    private _uploadShaderValue(): void {
        ...
        switch (this._shadowMapCount) {
            case 1:
                sceneSV.addDefine(
                    Scene3DShaderDeclaration.SHADERDEFINE_SHADOW_PSSM1,
                );
                sceneSV.removeDefine(
                    Scene3DShaderDeclaration.SHADERDEFINE_SHADOW_PSSM2,
                );
                sceneSV.removeDefine(
                    Scene3DShaderDeclaration.SHADERDEFINE_SHADOW_PSSM3,
                );
                break;
            case 2:
                sceneSV.addDefine(
        ...
        sceneSV.setVector(
            ILaya3D.Scene3D.SHADOWDISTANCE,
            this._shaderValueDistance,
        );
        sceneSV.setBuffer(
            ILaya3D.Scene3D.SHADOWLIGHTVIEWPROJECT,
            this._shaderValueLightVP,
        );
        sceneSV.setVector2(
            ILaya3D.Scene3D.SHADOWMAPPCFOFFSET,
            this._shadowPCFOffset,
        );
        switch (this._shadowMapCount) {
            case 3:
                sceneSV.setTexture(
                    ILaya3D.Scene3D.SHADOWMAPTEXTURE1,
                    this.cameras[1].renderTarget,
                );
                sceneSV.setTexture(
                    ILaya3D.Scene3D.SHADOWMAPTEXTURE2,
                    this.cameras[2].renderTarget,
                );
                sceneSV.setTexture(
                    ILaya3D.Scene3D.SHADOWMAPTEXTURE3,
                    this.cameras[3].renderTarget,
                );
                break;
            case 2:
                sceneSV.setTexture(
    ...
    private _calcSplitDistance(nearPlane: number): void {
        ...
        for (i = 0; i <= this._shadowMapCount; i++) {
            this._uniformDistance[i] =
                nearPlane + (far - nearPlane) * i * invNumberOfPSSM;
        }
        ...
        for (i = 0; i <= this._shadowMapCount; i++) {
            this._spiltDistance[i] =
                this._uniformDistance[i] * this._ratioOfDistance +
                this._logDistance[i] * (1.0 - this._ratioOfDistance);
        }

        this._shaderValueDistance.x =
            this._spiltDistance[1] != undefined && this._spiltDistance[1];
        this._shaderValueDistance.y =
            this._spiltDistance[2] != undefined && this._spiltDistance[2];
        this._shaderValueDistance.z =
            this._spiltDistance[3] != undefined && this._spiltDistance[3];
        this._shaderValueDistance.w =
            this._spiltDistance[4] != undefined && this._spiltDistance[4]; //_spiltDistance[4]为undefine 微信小游戏
    }
    ...
    calcSplitFrustum(sceneCamera: BaseCamera): void {
        if (this._currentPSSM > 0) {
            Matrix4x4.createPerspective(
                (3.1416 * sceneCamera.fieldOfView) / 180.0,
                (<Camera>sceneCamera).aspectRatio,
            ...
            Matrix4x4.createPerspective(
                ...
                this._spiltDistance[this._shadowMapCount],
                this._tempMatrix44,
            );
        }
        Matrix4x4.multiply(
            this._tempMatrix44,
            (<Camera>sceneCamera).viewMatrix,
            this._tempMatrix44,
        );
    ...
    private _rebuildRenderInfo(): void {
        ...
        for (i = 0; i < nNum; i++) {
            ...
            if (
                shadowMap == null ||
                shadowMap.width != this._shadowMapTextureSize ||
                shadowMap.height != this._shadowMapTextureSize
            ) {
                shadowMap && shadowMap.destroy();
                shadowMap = new RenderTexture(
                    this._shadowMapTextureSize,
                    this._shadowMapTextureSize,
                    RenderTextureFormat.R8G8B8A8,
    ...
    private _calcLightViewProject(sceneCamera: BaseCamera): void {
        ...
        sceneCamera.transform.worldMatrix.getForward(
            ParallelSplitShadowMap._tempVector30,
        );
        ...
        curLightCamera.transform.lookAt(
            this._tempLookAt3,
            this._tempLightUp,
            false,
        );
        ...
        for (var i: number = 0; i < 8; i++) {
            ...
            Vector4.transformByM4x4(
                this._tempValue,
                this._tempMatrix44,
                this._tempValue,
            );
            tempMin.x =
                tempValueElement.x < tempMin.x ? tempValueElement.x : tempMin.x;
            tempMin.y =
                tempValueElement.y < tempMin.y ? tempValueElement.y : tempMin.y;
            tempMin.z =
                tempValueElement.z < tempMin.z ? tempValueElement.z : tempMin.z;
            tempMax.x =
                tempValueElement.x > tempMax.x ? tempValueElement.x : tempMax.x;
            tempMax.y =
                tempValueElement.y > tempMax.y ? tempValueElement.y : tempMax.y;
            tempMax.z =
                tempValueElement.z > tempMax.z ? tempValueElement.z : tempMax.z;
        }
        ...
        Vector4.transformByM4x4(
            this._tempValue,
            curLightCamera.transform.worldMatrix,
            this._tempValue,
        );
        var distance: number = Math.abs(-this._tempMax.z);
        var farPlane: number =
            distance > this._maxDistance ? distance : this._maxDistance;
        ...
        Matrix4x4.createOrthoOffCenter(
            tempMin.x,
            tempMax.x,
            tempMin.y,
            tempMax.y,
            1.0,
            farPlane + 0.5 * (tempMax.z - tempMin.z),
            curLightCamera.projectionMatrix,
        );

        //calc frustum
        var projectView: Matrix4x4 = curLightCamera.projectionViewMatrix;
        ParallelSplitShadowMap.multiplyMatrixOutFloat32Array(
            this._tempScaleMatrix44,
            projectView,
            this._shaderValueVPs[this._currentPSSM],
        );
        this._scene._shaderValues.setBuffer(
            ILaya3D.Scene3D.SHADOWLIGHTVIEWPROJECT,
            this._shaderValueLightVP,
        );
    }
     ...
     * @param	out  输出矩阵
     */
    static multiplyMatrixOutFloat32Array(
        left: Matrix4x4,
        right: Matrix4x4,
        out: Float32Array,
    ): void {
