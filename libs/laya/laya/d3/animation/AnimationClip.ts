...
export class AnimationClip extends Resource {
    ...
    private _hermiteInterpolate(
        frame: FloatKeyframe,
        nextFrame: FloatKeyframe,
        t: number,
        dur: number,
    ): number {
        var t0: number = frame.outTangent,
            t1: number = nextFrame.inTangent;
    ...
    private _hermiteInterpolateVector3(
        frame: Vector3Keyframe,
        nextFrame: Vector3Keyframe,
        t: number,
        dur: number,
        out: Vector3,
    ): void {
    ...
    private _hermiteInterpolateQuaternion(
        frame: QuaternionKeyframe,
        nextFrame: QuaternionKeyframe,
        t: number,
        dur: number,
        out: Quaternion,
    ): void {
    ...
    _evaluateClipDatasRealTime(
        nodes: KeyframeNodeList,
        playCurTime: number,
        realTimeCurrentFrameIndexes: Int16Array,
        addtive: boolean,
        frontPlay: boolean,
    ): void {
        for (var i: number = 0, n: number = nodes.count; i < n; i++) {
            ...
            if (frontPlay) {
                if (
                    frameIndex !== -1 &&
                    playCurTime < keyFrames[frameIndex].time
                ) {
                    //重置正向循环
                    frameIndex = -1;
                ...
                nextFrameIndex = frameIndex + 1;
                if (
                    nextFrameIndex !== keyFramesCount &&
                    playCurTime > keyFrames[nextFrameIndex].time
                ) {
                    //重置逆向循环
            ...
            switch (type) {
                case 0:
                    if (frameIndex !== -1) {
                        var frame: FloatKeyframe = <FloatKeyframe>(
                            keyFrames[frameIndex]
                        );
                        if (isEnd) {
                            //如果nextFarme为空，不修改数据，保持上一帧
                            node.data = frame.value;
                        } else {
                            var nextFarme: FloatKeyframe = <FloatKeyframe>(
                            ...
                            node.data = this._hermiteInterpolate(
                                frame,
                                nextFarme,
                                t,
                                d,
                ...
                case 4:
                    ...
                    this._evaluateFrameNodeVector3DatasRealTime(
                        keyFrames as Vector3Keyframe[],
                        frameIndex,
                        isEnd,
                        playCurTime,
                        clipData,
                    ...
                    if (addtive) {
                        var firstFrameValue: Vector3 = (<Vector3Keyframe>(
                            keyFrames[0]
                        )).value;
                ...
                case 2:
                    ...
                    this._evaluateFrameNodeQuaternionDatasRealTime(
                        keyFrames as QuaternionKeyframe[],
                        frameIndex,
                        isEnd,
                        playCurTime,
                        clipQuat,
                    ...
                    if (addtive) {
                        var tempQuat: Quaternion =
                            AnimationClip._tempQuaternion0;
                        var firstFrameValueQua: Quaternion = (<
                            QuaternionKeyframe
                        >keyFrames[0]).value;
                        Utils3D.quaternionConjugate(
                            firstFrameValueQua,
                            tempQuat,
                        );
    ...
    _evaluateClipDatasRealTimeForNative(
        nodes: any,
        playCurTime: number,
        realTimeCurrentFrameIndexes: Uint16Array,
        addtive: boolean,
    ): void {
        (<any>LayaGL.instance).evaluateClipDatasRealTime(
            nodes._nativeObj,
            playCurTime,
    ...
    private _evaluateFrameNodeVector3DatasRealTime(
        ...
        if (frameIndex !== -1) {
            ...
            if (isEnd) {
                ...
                this._hermiteInterpolateVector3(
                    frame,
                    nextKeyFrame,
                    t,
                    d,
    ...
    private _evaluateFrameNodeQuaternionDatasRealTime(
        ...
        if (frameIndex !== -1) {
            ...
            if (isEnd) {
                ...
                var nextKeyFrame: QuaternionKeyframe =
                    keyFrames[frameIndex + 1];
                var t: number;
                var startTime: number = frame.time;
                var d: number = nextKeyFrame.time - startTime;
                if (d !== 0) t = (playCurTime - startTime) / d;
                ...
                this._hermiteInterpolateQuaternion(
                    frame,
                    nextKeyFrame,
                    t,
                    d,
