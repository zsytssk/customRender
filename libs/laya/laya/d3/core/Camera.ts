...
export class Camera extends BaseCamera {
    ...
    get viewport(): Viewport {
        ...
        if (this._offScreenRenderTexture)
            this._calculationViewport(
                this._normalizedViewport,
                this._offScreenRenderTexture.width,
                this._offScreenRenderTexture.height,
            );
        else
            this._calculationViewport(
                this._normalizedViewport,
                RenderContext3D.clientWidth,
                RenderContext3D.clientHeight,
            ); //屏幕尺寸会动态变化,需要重置
    ...
    get boundFrustum(): BoundFrustum {
        ...
        if (Render.supportWebGLPlusCulling) {
            ...
            (buffer[0] = nearNE.x),
                (buffer[1] = nearNE.y),
                (buffer[2] = nearNE.z),
                (buffer[3] = near.distance);
            (buffer[4] = farNE.x),
                (buffer[5] = farNE.y),
                (buffer[6] = farNE.z),
                (buffer[7] = far.distance);
            (buffer[8] = leftNE.x),
                (buffer[9] = leftNE.y),
                (buffer[10] = leftNE.z),
                (buffer[11] = left.distance);
            (buffer[12] = rightNE.x),
                (buffer[13] = rightNE.y),
                (buffer[14] = rightNE.z),
                (buffer[15] = right.distance);
            (buffer[16] = topNE.x),
                (buffer[17] = topNE.y),
                (buffer[18] = topNE.z),
                (buffer[19] = top.distance);
            (buffer[20] = bottomNE.x),
                (buffer[21] = bottomNE.y),
                (buffer[22] = bottomNE.z),
                (buffer[23] = bottom.distance);
        }
    ...
    set postProcess(value: PostProcess) {
        ...
        this.addCommandBuffer(
            Camera.CAMERAEVENT_POSTPROCESS,
    ...
    set enableHDR(value: boolean) {
        if (
            value &&
            !SystemUtils.supportRenderTextureFormat(
                RenderTextureFormat.R16G16B16A16,
            )
        ) {
    ...
    private _calculationViewport(
        ...
        var pixelLeftX: number =
            ceilLeftX - lx >= 0.5 ? Math.floor(lx) : ceilLeftX;
        var pixelLeftY: number =
            ceilLeftY - ly >= 0.5 ? Math.floor(ly) : ceilLeftY;
        var pixelRightX: number =
            rx - floorRightX >= 0.5 ? Math.ceil(rx) : floorRightX;
        var pixelRightY: number =
            ry - floorRightY >= 0.5 ? Math.ceil(ry) : floorRightY;
    ...
    _parse(data: any, spriteMap: any): void {
        ...
        this.normalizedViewport = new Viewport(
            viewport[0],
            viewport[1],
            viewport[2],
            viewport[3],
    ...
    _getCanvasWidth(): number {
        ...
        else return RenderContext3D.clientWidth;
    }
    ...
    _getCanvasHeight(): number {
        ...
        else return RenderContext3D.clientHeight;
    }
    ...
    _applyPostProcessCommandBuffers(): void {
        for (
            var i: number = 0,
                n: number = this._postProcessCommandBuffers.length;
            i < n;
            i++
        )
    ...
    _getRenderTextureFormat(): number {
        if (this._enableHDR) return RenderTextureFormat.R16G16B16A16;
        else return RenderTextureFormat.R8G8B8;
    }
    ...
    _applyViewProject(
        ...
        if (context.invertY) {
            Matrix4x4.multiply(
                BaseCamera._invertYScaleMatrix,
                proMat,
                BaseCamera._invertYProjectionMatrix,
            );
            Matrix4x4.multiply(
                BaseCamera._invertYProjectionMatrix,
                viewMat,
                BaseCamera._invertYProjectionViewMatrix,
            );
    ...
    _updateClusterPlaneXY(): void {
        ...
        if (
            ...
            var xSlixe: number = clusterCount.x,
                ySlice: number = clusterCount.y;
            var xCount: number = xSlixe + 1,
                yCount: number = ySlice + 1;
            var xPlanes: Vector3[] = this._clusterXPlanes,
                yPlanes: Vector3[] = this._clusterYPlanes;
    ...
    render(shader: Shader3D = null, replacementTag: string = null): void {
        if (!this.activeInHierarchy)
            //custom render should protected with activeInHierarchy=true
            return;
        ...
        if (needInternalRT)
            this._internalRenderTexture = RenderTexture.createFromPool(
                viewport.width,
                viewport.height,
                this._getRenderTextureFormat(),
                RenderTextureDepthFormat.DEPTH_16,
                BaseTexture.FILTERMODE_BILINEAR,
            );
        else this._internalRenderTexture = null;
        if (scene.parallelSplitShadowMaps[0]) {
            //TODO:SM
            ShaderData.setRuntimeValueMode(false);
            var parallelSplitShadowMap: ParallelSplitShadowMap =
                scene.parallelSplitShadowMaps[0];
            parallelSplitShadowMap._calcAllLightCameraInfo(this);
            scene._shaderValues.addDefine(
                Scene3DShaderDeclaration.SHADERDEFINE_CAST_SHADOW,
            ); //增加宏定义
            for (
                var i: number = 0,
                    n: number = parallelSplitShadowMap.shadowMapCount;
                i < n;
                i++
            ) {
                ...
                var shadowMap: RenderTexture =
                    parallelSplitShadowMap.cameras[i + 1].renderTarget;
                shadowMap._start();
            ...
            scene._shaderValues.removeDefine(
                Scene3DShaderDeclaration.SHADERDEFINE_CAST_SHADOW,
            ); //去掉宏定义
        ...
        if (
            needInternalRT &&
            !this._offScreenRenderTexture &&
            (this.clearFlag == CameraClearFlags.DepthOnly ||
                this.clearFlag == CameraClearFlags.Nothing)
        ) {
            if (this._enableHDR) {
                //internal RT is HDR can't directly copy
                var grabTexture: RenderTexture = RenderTexture.createFromPool(
                    viewport.width,
                    viewport.height,
                ...
                var blit: BlitScreenQuadCMD = BlitScreenQuadCMD.create(
                    grabTexture,
                    this._internalRenderTexture,
                );
                blit.run();
                blit.recover();
                ...
                WebGLContext.bindTexture(
                    gl,
                    gl.TEXTURE_2D,
                    this._internalRenderTexture._getSource(),
                );
                gl.copyTexSubImage2D(
                    gl.TEXTURE_2D,
                    0,
        ...
        if (needInternalRT) {
            if (this._postProcess) {
                ...
                var canvasWidth: number = this._getCanvasWidth(),
                    canvasHeight: number = this._getCanvasHeight();
                this._screenOffsetScale.setValue(
                    viewport.x / canvasWidth,
                    viewport.y / canvasHeight,
    ...
    convertScreenCoordToOrthographicCoord(
        ...
        //TODO:是否应该使用viewport宽高
        if (this._orthographic) {
            var clientWidth: number = RenderContext3D.clientWidth;
            var clientHeight: number = RenderContext3D.clientHeight;
            var ratioX: number =
                (this.orthographicVerticalSize * this.aspectRatio) /
                clientWidth;
            var ratioY: number = this.orthographicVerticalSize / clientHeight;
            out.x =
                (-clientWidth / 2 + source.x * Laya.stage.clientScaleX) *
                ratioX;
            out.y =
                (clientHeight / 2 - source.y * Laya.stage.clientScaleY) *
                ratioY;
            out.z =
                ((this.nearPlane - this.farPlane) * (source.z + 1)) / 2 -
                this.nearPlane;
    ...
    removeCommandBuffer(event: number, commandBuffer: CommandBuffer): void {
        switch (event) {
            case Camera.CAMERAEVENT_POSTPROCESS:
                var index: number = this._postProcessCommandBuffers.indexOf(
                    commandBuffer,
                );
                if (index !== -1)
