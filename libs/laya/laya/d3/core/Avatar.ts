...
export class Avatar extends Resource implements IClone {
    ...
    static _parse(
        ...
        if (Render.supportWebGLPlusAnimation) avatar._nativeNodeCount++; //[NATIVE]
        if (data.version) {
    ...
    private _initCloneToAnimator(
        destNode: AnimationNode,
        destAnimator: Animator,
    ): void {
        destAnimator._avatarNodeMap[destNode.name] = destNode;

        for (
            var i: number = 0, n: number = destNode.getChildCount();
            i < n;
            i++
        )
            this._initCloneToAnimator(
                destNode.getChildByIndex(i),
                destAnimator,
            );
    }
    ...
    private _parseNode(nodaData: any, node: AnimationNode): void {
        ...
        for (var j: number = 0, n: number = childrenData.length; j < n; j++) {
            ...
            if (Render.supportWebGLPlusAnimation) this._nativeNodeCount++; //[NATIVE]
    ...
    _cloneDatasToAnimatorNative(destAnimator: Animator): void {
        var animationNodeLocalPositions: Float32Array = new Float32Array(
            this._nativeNodeCount * 3,
        );
        var animationNodeLocalRotations: Float32Array = new Float32Array(
            this._nativeNodeCount * 4,
        );
        var animationNodeLocalScales: Float32Array = new Float32Array(
            this._nativeNodeCount * 3,
        );
        var animationNodeWorldMatrixs: Float32Array = new Float32Array(
            this._nativeNodeCount * 16,
        );
        var animationNodeParentIndices: Int16Array = new Int16Array(
        ...
        var destRoot: AnimationNode = this._rootNode._cloneNative(
            animationNodeLocalPositions,
            animationNodeLocalRotations,
            animationNodeLocalScales,
            animationNodeWorldMatrixs,
            animationNodeParentIndices,
            -1,
            this,
        );
