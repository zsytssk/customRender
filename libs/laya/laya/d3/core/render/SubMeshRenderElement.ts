...
export class SubMeshRenderElement extends RenderElement {
    ...
    _computeWorldPositionsAndNormals(
        ...
        if (this._dynamicWorldPositionNormalNeedUpdate) {
            ...
            var vertexFloatCount: number =
                vertexBuffer.vertexDeclaration.vertexStride / 4;
            ...
            for (var i: number = 0; i < vertexCount; i++) {
                ...
                Utils3D.transformVector3ArrayToVector3ArrayCoordinate(
                    oriVertexes,
                    oriOffset + positionOffset,
                    worldMat,
                    this._dynamicWorldPositions,
                    bakeOffset,
                );
                normalOffset !== -1 &&
                    Utils3D.transformVector3ArrayByQuat(
                        oriVertexes,
                        oriOffset + normalOffset,
                        rotation,
    ...
    setGeometry(geometry: GeometryElement): void {
        if (this._geometry !== geometry) {
            ...
            if (mesh) {
                //TODO:可能是StaticSubMesh
                var multiSubMesh: boolean = mesh._subMeshes.length > 1;
                var dynBatVerCount: number = multiSubMesh
                    ? subMesh._indexCount
                    : mesh._vertexCount;
                if (
                    dynBatVerCount <=
                    ILaya3D.SubMeshDynamicBatch.maxAllowVertexCount
                ) {
    ...
    addToOpaqueRenderQueue(context: RenderContext3D, queue: RenderQueue): void {
        var subMeshStaticBatch: SubMeshStaticBatch = <SubMeshStaticBatch>(
            this.staticBatch
        );
        var queueElements: SingletonList<RenderElement> = queue.elements;
        var elements: any[] = queueElements.elements;
        if (subMeshStaticBatch) {
            var staManager: MeshRenderStaticBatchManager =
                ILaya3D.MeshRenderStaticBatchManager.instance;
            var staBatchMarks: BatchMark = staManager.getBatchOpaquaMark(
                this.render.lightmapIndex + 1,
                this.render.receiveShadow,
                this.material.id,
                subMeshStaticBatch._batchID,
            );
            if (staManager._updateCountMark === staBatchMarks.updateMark) {
                ...
                if (staBatchMarks.batched) {
                    ...
                    var staOriElement: SubMeshRenderElement =
                        elements[staBatchIndex];
                    var staOriRender: BaseRender = staOriElement.render;
                    var staBatchElement: SubMeshRenderElement = <
                        SubMeshRenderElement
                    >staManager._getBatchRenderElementFromPool();
                    staBatchElement.renderType =
                        RenderElement.RENDERTYPE_STATICBATCH;
                    ...
                    var staBatchTransform: Transform3D = staRootOwner
                        ? staRootOwner._transform
                        : null;
                    staBatchElement.setTransform(staBatchTransform);
                    staBatchElement.render = staOriRender;
                    staBatchElement.renderSubShader =
                        staOriElement.renderSubShader;
                    var staBatchList: SingletonList<SubMeshRenderElement> =
                        staBatchElement.staticBatchElementList;
            ...
            this.renderSubShader._owner._enableInstancing &&
            LayaGL.layaGPUInstance.supportInstance() &&
            this.render.lightmapIndex < 0
        ) {
            //需要支持Instance渲染才可用,暂不支持光照贴图
            var subMesh: SubMesh = <SubMesh>this._geometry;
            var insManager: MeshRenderDynamicBatchManager =
                ILaya3D.MeshRenderDynamicBatchManager.instance;
            var insBatchMarks: BatchMark = insManager.getInstanceBatchOpaquaMark(
                this.render.receiveShadow,
                this.material.id,
                subMesh._id,
                this._transform._isFrontFaceInvert,
            );
            if (insManager._updateCountMark === insBatchMarks.updateMark) {
                ...
                if (insBatchMarks.batched) {
                    var instanceBatchElementList: SingletonList<SubMeshRenderElement> =
                        elements[insBatchIndex].instanceBatchElementList;
                    if (
                        instanceBatchElementList.length ===
                        SubMeshInstanceBatch.instance.maxInstanceCount
                    ) {
                    ...
                    var insOriElement: SubMeshRenderElement =
                        elements[insBatchIndex];
                    var insOriRender: BaseRender = insOriElement.render;
                    var insBatchElement: SubMeshRenderElement = <
                        SubMeshRenderElement
                    >insManager._getBatchRenderElementFromPool(); //TODO:是否动态和静态方法可合并
                    insBatchElement.renderType =
                    ...
                    insBatchElement.renderSubShader =
                        insOriElement.renderSubShader;
                    var insBatchList: SingletonList<SubMeshRenderElement> =
                        insBatchElement.instanceBatchElementList;
            ...
            var verDec: VertexDeclaration = (<SubMesh>this._geometry)
                ._vertexBuffer.vertexDeclaration;
            var dynManager: MeshRenderDynamicBatchManager =
                ILaya3D.MeshRenderDynamicBatchManager.instance;
            var dynBatchMarks: BatchMark = dynManager.getVertexBatchOpaquaMark(
                this.render.lightmapIndex + 1,
                this.render.receiveShadow,
                this.material.id,
                verDec.id,
            ...
            if (dynManager._updateCountMark === dynBatchMarks.updateMark) {
                ...
                if (dynBatchMarks.batched) {
                    ...
                    var dynOriElement: SubMeshRenderElement =
                        elements[dynBatchIndex];
                    var dynOriRender: BaseRender = dynOriElement.render;
                    var dynBatchElement: SubMeshRenderElement = <
                        SubMeshRenderElement
                    >dynManager._getBatchRenderElementFromPool(); //TODO:是否动态和静态方法可合并
                    dynBatchElement.renderType =
                        RenderElement.RENDERTYPE_VERTEXBATCH;
                    dynBatchElement.setGeometry(
                        ILaya3D.SubMeshDynamicBatch.instance,
                    );
                    ...
                    dynBatchElement.renderSubShader =
                        dynOriElement.renderSubShader;
                    var dynBatchList: SingletonList<SubMeshRenderElement> =
                        dynBatchElement.vertexBatchElementList;
    ...
    addToTransparentRenderQueue(
        ...
        if (subMeshStaticBatch) {
            var staManager: MeshRenderStaticBatchManager =
                ILaya3D.MeshRenderStaticBatchManager.instance;
            var staLastElement: RenderElement =
                queue.lastTransparentRenderElement;
            if (staLastElement) {
                var staLastRender: BaseRender = staLastElement.render;
                if (
                    staLastElement._geometry._getType() !==
                        this._geometry._getType() ||
                    staLastElement.staticBatch !== subMeshStaticBatch ||
                    staLastElement.material !== this.material ||
                    staLastRender.receiveShadow !== this.render.receiveShadow ||
                    staLastRender.lightmapIndex !== this.render.lightmapIndex
                ) {
                    queueElements.add(this);
                    ...
                    if (queue.lastTransparentBatched) {
                        (<SubMeshRenderElement>(
                            elements[queueElements.length - 1]
                        )).staticBatchElementList.add(this);
                    } else {
                        var staBatchElement: SubMeshRenderElement = <
                            SubMeshRenderElement
                        >staManager._getBatchRenderElementFromPool();
                        ...
                        var staRootOwner: Sprite3D =
                            subMeshStaticBatch.batchOwner;
                        var staBatchTransform: Transform3D = staRootOwner
                            ? staRootOwner._transform
                            : null;
                        staBatchElement.setTransform(staBatchTransform);
                        ...
                        staBatchElement.renderSubShader =
                            staLastElement.renderSubShader;
                        var staBatchList: SingletonList<SubMeshRenderElement> =
                            staBatchElement.staticBatchElementList;
                        staBatchList.length = 0;
            ...
            var insManager: MeshRenderDynamicBatchManager =
                ILaya3D.MeshRenderDynamicBatchManager.instance;
            var insLastElement: RenderElement =
                queue.lastTransparentRenderElement;
            if (insLastElement) {
                var insLastRender: BaseRender = insLastElement.render;
                if (
                    insLastElement._geometry._getType() !==
                        this._geometry._getType() ||
                    <SubMesh>insLastElement._geometry !== subMesh ||
                    insLastElement.material !== this.material ||
                    insLastRender.receiveShadow !== this.render.receiveShadow
                ) {
                    queueElements.add(this);
                    queue.lastTransparentBatched = false;
                } else {
                    if (queue.lastTransparentBatched) {
                        ...
                        var insBatchElement: SubMeshRenderElement = <
                            SubMeshRenderElement
                        >insManager._getBatchRenderElementFromPool();
                        insBatchElement.renderType =
                            RenderElement.RENDERTYPE_INSTANCEBATCH;
                        insBatchElement.setGeometry(
                            SubMeshInstanceBatch.instance,
                        );
                        ...
                        insBatchElement.instanceSubMesh = subMesh;
                        insBatchElement.renderSubShader =
                            insLastElement.renderSubShader;
                        var insBatchList: SingletonList<SubMeshRenderElement> =
                            insBatchElement.instanceBatchElementList;
                        insBatchList.length = 0;
            ...
            var dynManager: MeshRenderDynamicBatchManager =
                ILaya3D.MeshRenderDynamicBatchManager.instance;
            var dynLastElement: RenderElement =
                queue.lastTransparentRenderElement;
            if (dynLastElement) {
                var dynLastRender: BaseRender = dynLastElement.render;
                if (
                    dynLastElement._geometry._getType() !==
                        this._geometry._getType() ||
                    (<SubMesh>dynLastElement._geometry)._vertexBuffer
                        ._vertexDeclaration !== verDec ||
                    dynLastElement.material !== this.material ||
                    dynLastRender.receiveShadow !== this.render.receiveShadow ||
                    dynLastRender.lightmapIndex !== this.render.lightmapIndex
                ) {
                    queueElements.add(this);
                    queue.lastTransparentBatched = false;
                } else {
                    if (queue.lastTransparentBatched) {
                        (<SubMeshRenderElement>(
                            elements[queueElements.length - 1]
                        )).vertexBatchElementList.add(this);
                    } else {
                        var dynBatchElement: SubMeshRenderElement = <
                            SubMeshRenderElement
                        >dynManager._getBatchRenderElementFromPool();
                        dynBatchElement.renderType =
                            RenderElement.RENDERTYPE_VERTEXBATCH;
                        dynBatchElement.setGeometry(
                            ILaya3D.SubMeshDynamicBatch.instance,
                        ...
                        dynBatchElement.vertexBatchVertexDeclaration = verDec;
                        dynBatchElement.renderSubShader =
                            dynLastElement.renderSubShader;
                        var dynBatchList: SingletonList<SubMeshRenderElement> =
                            dynBatchElement.vertexBatchElementList;
                        dynBatchList.length = 0;
