...
export class ShaderPass extends ShaderCompile {
            ...
            if ((ofs = text.indexOf('#')) >= 0) {
                ...
                switch (name) {
                    ...
                    case '#ifndef':
                        ...
                        if (defs) {
                            words = text
                                .substr(j)
                                .split(ShaderCompile._splitToWordExps3);
                    ...
                    case '#include': //这里有问题,主要是空格
                        ...
                        var inlcudeFile: InlcudeFile =
                            ShaderCompile.includes[words[1]];
                        if (!inlcudeFile) {
                            throw (
                        ...
                        if ((ofs = words[0].indexOf('?')) < 0) {
                            node.setParent(parent);
                            text = inlcudeFile.getWith(
                                words[2] == 'with' ? words[3] : null,
                            );
                            this._compileToTree(
                                node,
                                text.split('\n'),
                        ...
                        node.setCondition(
                            words[0].substr(ofs + 1),
                            ShaderCompile.IFDEF_YES,
                        );
                        node.text = inlcudeFile.getWith(
                            words[2] == 'with' ? words[3] : null,
                        );
                        break;
                    case '#import':
                ...
                if (preNode && !preNode.name) {
                    includefiles.length > 0 &&
                        ShaderCompile.splitToWords(text, preNode);
                    noUseNode = node;
                    preNode.text += '\n' + text;
                    continue;
                }
                includefiles.length > 0 &&
                    ShaderCompile.splitToWords(text, node);
            }
            node.setParent(parent);
        }
    }
    ...
    _resizeCacheShaderMap(
        ...
        if (hierarchy == end) {
            for (var k in cacheMap) {
                ...
                for (
                    var i: number = 0, n: number = resizeLength - end;
                    i < n;
                    i++
                ) {
                    if (i == n - 1) cacheMap[0] = shader;
                    //0替代(i == 0 ? k : 0),只扩不缩
                    else cacheMap = cacheMap[i == 0 ? k : 0] = {};
                }
            }
    ...
    _addDebugShaderVariantCollection(compileDefine: DefineDatas): void {
        var dbugShaderVariantInfo: ShaderVariant =
        ...
        if (dbugShaderVariantInfo)
            dbugShaderVariantInfo.setValue(
                debugShader,
                debugShader._subShaders.indexOf(debugSubShader),
                debugSubShader._passes.indexOf(this),
                deugDefines,
            );
        else
            Shader3D._debugShaderVariantInfo = dbugShaderVariantInfo = new ShaderVariant(
                debugShader,
                debugShader._subShaders.indexOf(debugSubShader),
                debugSubShader._passes.indexOf(this),
                deugDefines,
    ...
    withCompile(compileDefine: DefineDatas): ShaderInstance {
        ...
        if (Shader3D.debugMode)
            //add shader variant info to debug ShaderVariantCollection
        ...
        defineStr +=
            '#define MAX_LIGHT_COUNT_PER_CLUSTER ' +
            config._maxAreaLightCountPerClusterAverage +
            '\n';
        ...
        shader = new ShaderInstance(
            vsVersion + defineStr + vs.join('\n'),
            psVersion + defineStr + ps.join('\n'),
            this._owner._attributeMap || this._owner._owner._attributeMap,
            this._owner._uniformMap || this._owner._owner._uniformMap,
            this,
        );
        ...
        if (Shader3D.debugMode) {
            ...
            if (!config._multiLighting) {
                compileDefine.remove(
                    Shader3D.SHADERDEFINE_LEGACYSINGALLIGHTING,
                );
            ...
            for (
                var i: number = 0, n: number = compileDefine._length;
                i < n;
                i++
            )
            ...
            for (var i: number = 0, n: number = defineString.length; i < n; i++)
                i == n - 1
                    ? (defStr += defineString[i])
                    : (defStr += defineString[i] + ',');

            console.log(
                '%cLayaAir: Shader Compile Information---ShaderName:' +
                    this._owner._owner._name +
                    ' SubShaderIndex:' +
                    this._owner._owner._subShaders.indexOf(this._owner) +
                    ' PassIndex:' +
                    this._owner._passes.indexOf(this) +
                    ' DefineMask:[' +
                    defMask +
                    ']' +
                    ' DefineNames:[' +
