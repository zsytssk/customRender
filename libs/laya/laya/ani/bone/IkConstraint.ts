...
export class IkConstraint {
    ...
    constructor(data: IkConstraintData, bones: Bone[]) {
        ...
        for (
            var i: number = 0, n: number = data.boneIndexs.length;
            i < n;
            i++
        ) {
    ...
    private _applyIk1(
        ...
        var id: number =
            1 /
            (pp.resultMatrix.a * pp.resultMatrix.d -
                pp.resultMatrix.b * pp.resultMatrix.c);
        var x: number = targetX - pp.resultMatrix.tx;
        var y: number = targetY - pp.resultMatrix.ty;
        var tx: number =
            (x * pp.resultMatrix.d - y * pp.resultMatrix.c) * id -
            bone.transform.x;
        var ty: number =
            (y * pp.resultMatrix.a - x * pp.resultMatrix.b) * id -
            bone.transform.y;
        var rotationIK: number =
            Math.atan2(ty, tx) * IkConstraint.radDeg - 0 - bone.transform.skX;
        if (bone.transform.scX < 0) rotationIK += 180;
        if (rotationIK > 180) rotationIK -= 360;
        else if (rotationIK < -180) rotationIK += 360;
        bone.transform.skX = bone.transform.skY =
            bone.transform.skX + rotationIK * alpha;
        bone.update();
    }
    ...
    private _applyIk2(
        ...
        //spine 双骨骼ik计算
        var px: number = parent.resultTransform.x,
            py: number = parent.resultTransform.y;
        var psx: number = parent.transform.scX,
            psy: number = parent.transform.scY;
        ...
        var cx: number = child.resultTransform.x,
            cy: number,
            cwx: number,
            cwy: number;
        var a: number = parent.resultMatrix.a,
            b: number = parent.resultMatrix.c;
        var c: number = parent.resultMatrix.b,
            d: number = parent.resultMatrix.d;
        ...
        parent.setRotation(
            Math.atan2(
                cwy - parent.resultMatrix.ty,
                cwx - parent.resultMatrix.tx,
            ),
        );
        ...
        //求得IK中的子骨骼角度向量
        var x: number = targetX - pp.resultMatrix.tx,
            y: number = targetY - pp.resultMatrix.ty;
        ...
        if (u) {
            ...
            var aa: number = a * a,
                bb: number = b * b,
                dd: number = tx * tx + ty * ty,
                ta: number = Math.atan2(ty, tx);
            ...
            var minAngle: number = 0,
                minDist: number = Number.MAX_VALUE,
                minX: number = 0,
                minY: number = 0;
        ...
        parent.resultTransform.skX = parent.resultTransform.skY =
            rotation + a1 * alpha;
        ...
        child.resultTransform.skX = child.resultTransform.skY =
            child.resultTransform.skY + a2 * alpha;
        parent.update();
    }
    ...
    private _applyIk3(
        ...
        pTarMatrix.scale(
            parent.resultMatrix.getScaleX(),
            parent.resultMatrix.getScaleY(),
        );
        ...
        childTarMatrix.scale(
            child.resultMatrix.getScaleX(),
            child.resultMatrix.getScaleY(),
        );
