...
export class Templet extends AnimationTemplet {
    ...
    private onComplete(content: any = null): void {
        ...
        this._path =
            this._skBufferUrl.slice(0, this._skBufferUrl.lastIndexOf('/')) +
            '/';
    ...
    private _parseTexturePath(): void {
        ...
        ILaya.loader.load(
            this._loadList,
            Handler.create(this, this._textureComplete),
        );
    }
    ...
    private _textureComplete(): void {
        ...
        for (var i: number = 0, n: number = this._loadList.length; i < n; i++) {
            ...
            tTexture = this._textureDic[tTextureName] = ILaya.Loader.getRes(
                tTextureName,
            );
        }
        this._parsePublicExtData();
    ...
    private _parsePublicExtData(): void {
        ...
        var tX: number = 0,
            ...
            tWidth: number = 0,
            tHeight: number = 0;
        var tFrameX: number = 0,
            tFrameY: number = 0,
            tFrameWidth: number = 0,
            tFrameHeight: number = 0;
        var tTempleData: number = 0;
        ...
        for (i = 0; i < tTextureLen; i++) {
            ...
            tTempleData = tByte.getFloat32();
            tFrameHeight = isNaN(tTempleData) ? tHeight : tTempleData;
            this.subTextureDic[tTextureName] = Texture.create(
                tTexture,
                tX,
                tY,
                tWidth,
            ...
            for (var f: number = 0; f < tDeformSkinLen; f++) {
                ...
                for (j = 0; j < tDeformSlotLen; j++) {
                    ...
                    for (k = 0; k < tDeformSlotDisplayLen; k++) {
                        ...
                        tDeformSlotData.deformSlotDisplayList.push(
                        ...
                        for (l = 0; l < tDeformTimeLen; l++) {
                            ...
                            tDeformVectices = [];
                            tDeformSlotDisplayData.vectices.push(
                                tDeformVectices,
                            );
        ...
        var tSkinData: SkinData,
            tSlotData: SlotData,
            tDisplayData: SkinSlotDisplayData;
    ...
    showSkinByIndex(
        boneSlotDic: any,
        skinIndex: number,
        freshDisplayIndex: boolean = true,
    ): boolean {
            ...
            for (i = 0, n = tSkinData.slotArr.length; i < n; i++) {
                ...
                if (tSlotData) {
                    ...
                    if (tBoneSlot) {
                        ...
                        if (
                            freshDisplayIndex &&
                            tBoneSlot.attachmentName != 'undefined' &&
                            tBoneSlot.attachmentName != 'null'
                        ) {
                            tBoneSlot.showDisplayByName(
                                tBoneSlot.attachmentName,
                            );
                        } else {
                            tBoneSlot.showDisplayByIndex(
                                tBoneSlot.displayIndex,
                            );
                        }
                    }
                }
    ...
    getSkinIndexByName(skinName: string): number {
        var tSkinData: SkinData;
        for (
            var i: number = 0, n: number = this.skinDataArray.length;
            i < n;
            i++
        ) {
    ...
    getGrahicsDataWithCache(aniIndex: number, frameIndex: number): Graphics {
        if (
            this._graphicsCache[aniIndex] &&
            this._graphicsCache[aniIndex][frameIndex]
        ) {
    ...
    destroy(): void {
        ...
        for (
            var i: number = 0, n: number = this.skinSlotDisplayDataArr.length;
            i < n;
            i++
        ) {
