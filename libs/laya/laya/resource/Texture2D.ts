...
export class Texture2D extends BaseTexture {
    ...
    static _parse(
        ...
        var texture: Texture2D = constructParams
            ? new Texture2D(
                  constructParams[0],
                  constructParams[1],
                  constructParams[2],
    ...
    constructor(
        ...
        if (mipmap) {
            ...
            if (!compress) this._setPixels(null, 0, width, height); //init 0 level
            this._mipmapCount = 1;
    ...
    private _gpuCompressFormat(): boolean {
        return (
            this._format == TextureFormat.DXT1 ||
    ...
    private _pharseDDS(arrayBuffer: ArrayBuffer): void {
        ...
        var header: Int32Array = new Int32Array(
            arrayBuffer,
            0,
            DDS_HEADER_LENGTH,
        );
        ...
        if (header[DDS_HEADER_FLAGS] & DDSD_MIPMAPCOUNT) {
            ...
            if (!this._mipmap) throw 'the mipmap is not same with Texture2D.';
        } else {
            if (this._mipmap) throw 'the mipmap is not same with Texture2D.';
        }
        ...
        this._upLoadCompressedTexImage2D(
            arrayBuffer,
            width,
            height,
            mipLevels,
    ...
    private _pharseKTX(arrayBuffer: ArrayBuffer): void {
        ...
        switch (compressedFormat) {
            case LayaGL.layaGPUInstance._compressedTextureEtc1
    ...
    _upLoadCompressedTexImage2D(
        ...
        for (var i: number = 0; i < miplevelCount; i++) {
            ...
            var mipDataSize: number = this._calcualatesCompressedDataSize(
                this._format,
                width,
                height,
            );
    ...
    loadImageSource(source: any, premultiplyAlpha: boolean = false): void {
        ...
        this._width = width;
        this._height = height;
        if (!(this._isPot(width) && this._isPot(height))) this._mipmap = false;
        ...
        if (this._canRead) {
            ...
            if (ILaya.Render.isConchApp) {
                this._pixels = new Uint8Array(
                    source._nativeObj.getImageData(0, 0, width, height),
                ); //TODO:如果为RGB,会错误
            } else {
                ...
                this._pixels = new Uint8Array(
                    ILaya.Browser.context.getImageData(
                        0,
                        0,
                        width,
                        height,
    ...
    setPixels(pixels: Uint8Array | Float32Array, miplevel: number = 0): void {
        ...
        if (pixels.length < pixelsCount)
            throw (
                'Texture2D:pixels length should at least ' + pixelsCount + '.'
            );
        this._setPixels(pixels, miplevel, width, height);

        if (this._canRead) this._pixels = pixels;
    ...
    getPixels(): Uint8Array | Float32Array {
        ...
        else throw new Error('Texture2D: must set texture canRead is true.');
    }
}
