...
export class LayaGPU {
    ...
    constructor(gl: any, isWebGL2: boolean) {
        ...
        if (!isWebGL2) {
            ...
            if (!ILaya.Render.isConchApp) {
                ...
                if ((window as any)._setupVertexArrayObject) {
                    //兼容VAO
                    if (forceVAO)
                        (window as any)._forceSetupVertexArrayObject(gl);
                    else (window as any)._setupVertexArrayObject(gl);
                }
            }
    ...
    createVertexArray(): any {
        if (this._isWebGL2) return this._gl.createVertexArray();
        else return this._vaoExt.createVertexArrayOES();
    }
    ...
    bindVertexArray(vertexArray: any): void {
        if (this._isWebGL2) this._gl.bindVertexArray(vertexArray);
        else this._vaoExt.bindVertexArrayOES(vertexArray);
    }
    ...
    deleteVertexArray(vertexArray: any): void {
        if (this._isWebGL2) this._gl.deleteVertexArray(vertexArray);
        else this._vaoExt.deleteVertexArrayOES(vertexArray);
    }
    ...
    isVertexArray(vertexArray: any): void {
        if (this._isWebGL2) this._gl.isVertexArray(vertexArray);
        else this._vaoExt.isVertexArrayOES(vertexArray);
    }
    ...
    drawElementsInstanced(
        ...
        if (this._isWebGL2)
            ...
            this._angleInstancedArrays.drawElementsInstancedANGLE(
                mode,
                count,
                type,
                offset,
    ...
    drawArraysInstanced(
        ...
        if (this._isWebGL2)
            ...
            this._angleInstancedArrays.drawArraysInstancedANGLE(
                mode,
                first,
                count,
                instanceCount,
    ...
    vertexAttribDivisor(index: number, divisor: number): void {
        if (this._isWebGL2) this._gl.vertexAttribDivisor(index, divisor);
        else
    ...
    supportInstance(): boolean {
        if (this._isWebGL2 || this._angleInstancedArrays) return true;
        else return false;
    }
