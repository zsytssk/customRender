...
export class Config3D implements IClone {
    ...
    set lightClusterCount(value: Vector3) {
        if (value.x > 128 || value.y > 128 || value.z > 128) {
            this._lightClusterCount.setValue(
                Math.min(value.x, 128),
                Math.min(value.y, 128),
                Math.min(value.z, 128),
            );
            console.warn(
                'Config3D: lightClusterCount X and Y、Z must less equal 128.',
            );
        } else {
            value.cloneTo(this._lightClusterCount);
        }

        var maxAreaLightCountWithZ =
            Math.floor(2048 / this._lightClusterCount.z - 1) * 4;
        if (maxAreaLightCountWithZ < this._maxLightCount)
            console.warn(
                'Config3D: if the area light(PointLight、SpotLight) count is large than ' +
                    maxAreaLightCountWithZ +
                    ',maybe the far away culster will ingonre some light.',
            );
        this._maxAreaLightCountPerClusterAverage = Math.min(
            maxAreaLightCountWithZ,
            this._maxLightCount,
        );
    }
