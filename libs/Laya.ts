...
export class Laya {
    ...
    static init(width: number, height: number, ...plugins): any {
        ...
        Browser.context = <CanvasRenderingContext2D>(
            (Browser.canvas.getContext('2d') as any)
        );
        ...
        Value2D._initone(
            ShaderDefines2D.TEXTURE2D | ShaderDefines2D.FILTERGLOW,
            TextureSV,
        );
    ...
    static set alertGlobalError(value: boolean) {
        ...
        if (value) {
            Browser.window.onerror = function (
                msg: string,
                url: string,
                line: string,
                column: string,
     ...
     */
    static enableDebugPanel(
        debugJsPath: string = 'libs/laya.debugtool.js',
    ): void {
    ...
    private static enableNative(): void {
        if (Laya.isNativeRender_enable) return;
        ...
        Sprite.drawToCanvas = Sprite.drawToTexture = function (
            sprite: Sprite,
            _renderType: number,
            canvasWidth: number,
            canvasHeight: number,
            ...
            ctx.asBitmap = true;
            ctx._targets.start();
            RenderSprite.renders[_renderType]._fun(
                sprite,
                ctx,
                offsetX,
                offsetY,
